...
public class MainActivity extends AppCompatActivity {
    ...
    public void extendTimer(long timeToAddMillis) {
        if (timerRunning) {
            pauseTimer(); // 현재 타이머 일시 정지
            timeLeftInMillis += timeToAddMillis; // 시간 추가
            ...
            // 타이머 바 애니메이션 재시작 (남은 시간과 새로운 길이로)
            int fullWidth = getResources().getDisplayMetrics().widthPixels;
            // 현재 타이머 바의 폭을 기준으로 남은 시간 비율을 계산
            int currentTimerBarWidth = timerBar.getWidth();
            long newDurationForAnim = timeLeftInMillis; // 새로운 애니메이션 총 시간

            if (barAnim != null) barAnim.cancel(); // 기존 애니메이션 취소

            barAnim = ValueAnimator.ofInt(currentTimerBarWidth, 0); // 현재 너비부터 0까지
            barAnim.setDuration(newDurationForAnim); // 새로운 시간으로 애니메이션 지속 시간 설정
            barAnim.setInterpolator(new LinearInterpolator());
            barAnim.addUpdateListener(animation -> {
                int width = (int) animation.getAnimatedValue();
                LinearLayout.LayoutParams params = new LinearLayout.LayoutParams(width, LinearLayout.LayoutParams.MATCH_PARENT);
            ...
            resumeTimer(); // 연장된 시간으로 타이머 재시작
        } else {
            // 게임이 시작되지 않았거나 종료된 경우, 타이머를 다시 시작할 필요가 있음
            // 이 경우, extendTimer는 restartGame()이나 startTimer() 후에 호출되어야 합니다.
            // 여기서는 단순하게 시간을 추가하는 것만 고려합니다.
    ...
    public void initializeGameField() {
        ...
        // 돌하르방 추가: 3개 무작위 위치에 추가
        int dolhareubangCount = 3; // 돌하르방 개수를 3개로 설정
        Set<Integer> dolhareubangIndices = new HashSet<>(); // 돌하르방 위치 저장을 위한 Set
        ...
        for (int i = 0; i < 54; i++) {
            ...
            if (dolhareubangIndices.contains(i)) { // 해당 인덱스가 돌하르방 인덱스에 포함되면
                num = 0; // 돌하르방은 0으로 지정
                block = new mandarin(this, num);
                block.markAsDolhareubang(); // 돌하르방으로 마크
            } else {
